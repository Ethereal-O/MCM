...
class Cellular(object):
    def __init__(self):
        # 应对策略
        self.strategy = "normal"  # "random"
        self.isExtendProtect = True  # False
    ...
    def update(self, n_iter):
        ...
        self.grass_fs = np.zeros(n_iter)
        self.herbivores_fs = np.zeros(n_iter)
        self.carnivores_fs = np.zeros(n_iter)
        for i in range(n_iter):
            ...
            self.grass_fs[i] = Caculate.caculateFinalScore(self.grass_cells)
            self.herbivores_fs[i] = Caculate.caculateFinalScore(
                self.herbivores_cells)
            self.carnivores_fs[i] = Caculate.caculateFinalScore(
                self.carnivores_cells)
...
class ProcessData(object):
    ...
    def getGrassData():
        cells = np.array(ProcessData.getStatus(
            ProcessData.perfix + "_grass.jpg.txt")).reshape((70, 80))
    ...
    def getHerbivoresData():
        cells = np.array(ProcessData.getStatus(
            ProcessData.perfix + "_herbivores.jpg.txt")).reshape((70, 80))
        for i in range(cells.shape[0]):
            for j in range(cells.shape[1]):
                if cells[i][j] == 5:
                    cells[i][j] = 0
    ...
    def getCarnivoresData():
        cells = np.array(ProcessData.getStatus(
            ProcessData.perfix + "_carnivores.jpg.txt")).reshape((70, 80))
        for i in range(cells.shape[0]):
            for j in range(cells.shape[1]):
                if cells[i][j] == 5:
                    cells[i][j] = 0
    ...
    def getHumansData():
        cells = np.array(ProcessData.getStatus(
            ProcessData.perfix + "_human.jpg.txt")).reshape((70, 80))
        for i in range(cells.shape[0]):
            for j in range(cells.shape[1]):
                if cells[i][j] == 7:
                    cells[i][j] = 1
...
class Caculate(object):
    ...
    w = 0.6 #w = 0.4 # w = 0.9  # 0.6 #0.9  # 保护区草地种植0.9 #0.6# 草生长系数
    n1 = 0.001  # 食草动物自我竞争影响
    n2 = 0.001  # 食肉动物自我竞争影响
    # human_effect_herbivores = 0
    # human_effect_carnivores = 0
    human_effect_herbivores = -0.05  # 电网增强-0.1#-0.05
    human_effect_carnivores = -0.005  # 电网增强-0.01#-0.005
    migration_rates = 1
    migration_num = 300
    ...
    def caculateHumansEffect(humans_cells, mode="normal"):
        ...
        # return np.loadtxt("./human_effect/human_effect_cells_"+mode+".txt", delimiter=','), np.loadtxt("./human_effect/human_effect_pos_"+mode+".txt", delimiter=',', dtype=bool)
    ...
    def caculateFinalScore(cells):
        x = 0
        y = 0
        fs = 0
        tot = np.sum(cells)
        for i in range(cells.shape[0]):
            x += np.sum(cells[i, :])*i/tot
        for j in range(cells.shape[1]):
            y += np.sum(cells[:, j])*j/tot

        for i in range(cells.shape[0]):
            for j in range(cells.shape[1]):
                fs += math.sqrt(Caculate.caculateDistance(i,
                                j, x, y))*cells[i][j]
...
if __name__ == '__main__':
    ...
    # print(game.grass_history)
    # print(game.herbivores_history)
    # print(game.carnivores_history)
    # print(game.grass_fs)
    # print(game.herbivores_fs)
    # print(game.carnivores_fs)
    ...
    lists = []
    tmp_lists = []
    tmp_lists.append(np.mean(game.grass_history))
    tmp_lists.append(np.mean(game.grass_fs))
    tmp_lists.append(np.mean(game.herbivores_history))
    tmp_lists.append(np.mean(game.herbivores_fs))
    tmp_lists.append(np.mean(game.carnivores_history))
    tmp_lists.append(np.mean(game.carnivores_fs))
    tmp_lists.append(game.human_cost)
    lists.append(tmp_lists)

    tmp_lists_2 = []
    tmp_lists_2.append(game.grass_history[-1])
    tmp_lists_2.append(game.grass_fs[-1])
    tmp_lists_2.append(game.herbivores_history[-1])
    tmp_lists_2.append(game.herbivores_fs[-1])
    tmp_lists_2.append(game.carnivores_history[-1])
    tmp_lists_2.append(game.carnivores_fs[-1])
    tmp_lists_2.append(game.human_cost)
    lists.append(tmp_lists_2)

    with open("./result/tot_其他国家临时.csv", "w", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerows(lists)

    plt.title('Iter :{}'.format(game.timer))
    plt.subplot(231)
    plt.imshow(game.visulize(game.grass_cells), cmap="Greens")
    plt.subplot(232)
    plt.imshow(game.visulize(game.herbivores_cells), cmap="Blues")
    plt.subplot(233)
    plt.imshow(game.visulize(game.carnivores_cells), cmap="Reds")
    plt.subplot(234)
    plt.plot(game.month, game.grass_history)
    plt.subplot(235)
    plt.plot(game.month, game.herbivores_history)
    plt.subplot(236)
    plt.plot(game.month, game.carnivores_history)
    # plt.imshow(game.visulize(game.grass_cells), cmap="Greens")
    # plt.imshow(game.visulize(game.herbivores_cells), cmap="Blues")
    # plt.imshow(game.visulize(game.carnivores_cells), cmap="Reds")
    ...
    # with open("./result/tot_迭代national.csv", "w", encoding="utf-8") as f:
    #     writer = csv.writer(f)
    #     writer.writerows(lists)
